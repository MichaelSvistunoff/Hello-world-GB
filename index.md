...
3. Переходим в папку проекта <pre>cd <repository_name></pre> \* Где <repository_name> - имя репозитория, полученное после **Fork**. После

5. Создаем необходимые ветки для редактирования <pre>`git` branch <new_branch_name></pre>

4. Редактируем контент. Сохраняем файл и создаем коммиты: <pre>`git` add <file_name.extension><br>`git` commit -m "message"</pre> \* *чем больше, тем лучше!*
